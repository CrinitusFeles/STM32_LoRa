...
#define SDMMC_PWR_OFF           0x00 // Power off: the clock to card is stopped
#define SDMMC_PWR_ON            0x03 // Power on: the card is clocked

// SDIO clock divider
#define SDMMC_CLK_DIV_400K      0x76 // SDIO clock 400kHz (48MHz / (0x76 + 2) = 400kHz)
#define SDMMC_CLK_DIV_1M        0x2e // SDIO clock 1MHz (48MHz / (0x2e + 2) = 1MHz)
#define SDMMC_CLK_DIV_2M        0x16 // SDIO clock 1MHz (48MHz / (0x16 + 2) = 2MHz)
#define SDMMC_CLK_DIV_4M        0x0a // SDIO clock 1MHz (48MHz / (0x0a + 2) = 4MHz)
#define SDMMC_CLK_DIV_6M85      0x05 // SDIO clock 1MHz (48MHz / (0x05 + 2) = 6.85MHz)
#define SDMMC_CLK_DIV_8M        0x04 // SDIO clock 1MHz (48MHz / (0x04 + 2) = 8MHz)
#define SDMMC_CLK_DIV_9M6       0x03 // SDIO clock 1MHz (48MHz / (0x03 + 2) = 9.6MHz)
#define SDMMC_CLK_DIV_12M       0x02 // SDIO clock 1MHz (48MHz / (0x02 + 2) = 12MHz)
#define SDMMC_CLK_DIV_16M       0x01 // SDIO clock 1MHz (48MHz / (0x01 + 2) = 16MHz)
#define SDMMC_CLK_DIV_24M       0x00 // SDIO clock 1MHz (48MHz / (0x00 + 2) = 12MHz)

// SDIO clocks for initialization and data transfer
#define SDMMC_CLK_DIV_INIT      SDMMC_CLK_DIV_400K // SDIO initialization frequency (400kHz)
//#define SDMMC_CLK_DIV_TRAN      SDMMC_CLK_DIV_400K  // SDIO data transfer 400kHz
//#define SDMMC_CLK_DIV_TRAN      SDMMC_CLK_DIV_1M  // SDIO data transfer 1MHz
//#define SDMMC_CLK_DIV_TRAN      SDMMC_CLK_DIV_8M  // SDIO data transfer 8MHz
//#define SDMMC_CLK_DIV_TRAN      SDMMC_CLK_DIV_9M6  // SDIO data transfer 9.6MHz
//#define SDMMC_CLK_DIV_TRAN      SDMMC_CLK_DIV_12M  // SDIO data transfer 12MHz
//#define SDMMC_CLK_DIV_TRAN      SDMMC_CLK_DIV_16M  // SDIO data transfer 16MHz
#define SDMMC_CLK_DIV_TRAN      SDMMC_CLK_DIV_24M  // SDIO data transfer 24MHz

// SDIO CMD response type
#define SDMMC_RESP_NONE         0x00                // No response
#define SDMMC_RESP_SHORT        SDMMC_CMD_WAITRESP_0 // Short response
#define SDMMC_RESP_LONG         SDMMC_CMD_WAITRESP   // Long response
...
#define SDMMC_CMD_TIMEOUT              ((uint32_t)0x00010000)

// SDIO timeout for data transfer ((48MHz / CLKDIV / 1000) * timeout_ms)
#define SDMMC_DATA_R_TIMEOUT           ((uint32_t)((48000000 / (SDMMC_CLK_DIV_TRAN + 2) / 1000) * 100)) // Data read timeout is 100ms
#define SDMMC_DATA_W_TIMEOUT           ((uint32_t)((48000000 / (SDMMC_CLK_DIV_TRAN + 2) / 1000) * 250)) // Date write timeout is 250ms
//#define SDMMC_DATA_R_TIMEOUT           ((uint32_t)0x00249F00) // Data read timeout is 100ms (24MHz SDIO)
//#define SDMMC_DATA_W_TIMEOUT           ((uint32_t)0x005B8D80) // Date write timeout is 250ms (24MHz SDIO)

// Trials count for ACMD41
#define SDMMC_ACMD41_TRIALS            ((uint32_t)0x0000FFFF)

// Bitmap to clear the SDIO static flags (command and data)
#define SDMMC_ICR_STATIC               ((uint32_t)(SDMMC_ICR_CCRCFAILC | SDMMC_ICR_DCRCFAILC | SDMMC_ICR_CTIMEOUTC | \
                                                  SDMMC_ICR_DTIMEOUTC | SDMMC_ICR_TXUNDERRC | SDMMC_ICR_RXOVERRC  | \
                                                  SDMMC_ICR_CMDRENDC  | SDMMC_ICR_CMDSENTC  | SDMMC_ICR_DATAENDC  | \
                                                  SDMMC_ICR_DBCKENDC))

// SDIO bus width
#define SD_BUS_1BIT                   0 // 1-bit wide bus (SDMMC_D0 used)
#define SD_BUS_4BIT                   SDMMC_CLKCR_WIDBUS_0 // 4-bit wide bus (SDMMC_D[3:0] used)
#define SD_BUS_8BIT                   SDMMC_CLKCR_WIDBUS_1 // 8-bit wide bus (SDMMC_D[7:0] used)

// SDIO transfer control flags
#define SDMMC_XFER_FLAGS               (SDMMC_STA_TXUNDERR | SDMMC_STA_DCRCFAIL | SDMMC_STA_DTIMEOUT | \
		                               SDMMC_STA_STBITERR | SDMMC_STA_DBCKEND  | SDMMC_STA_DATAEND)
